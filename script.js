...
function resetSelection(playerNum) {
    if (playerNum === 1) {
        ...
        if(p1Preview) {
            ...
            p1Preview.innerHTML = `<span style="color: #ffffff; font-weight: bold;">顯示 P1 棋子圖片</span>`;
        }
        ...
        if(p2Preview) {
            ...
            p2Preview.innerHTML = `<span style="color: #ffffff; font-weight: bold;">顯示 P2 棋子圖片</span>`;
...
// --- 2. 重組句子 ---
const reorderTasks = [ 
    { type: "reorder", question: "請排列出正確的句子：", words: ["操場上", "在", "小明", "今天", "跑步", "，", "。"], answer: "今天，小明在操場上跑步。" },
    { type: "reorder", question: "請排列出正確的句子：", words: ["在", "禮堂", "哥哥", "星期一", "表演跳舞", "，", "。"], answer: "星期一，哥哥在禮堂表演跳舞。" },
    { type: "reorder", question: "請排列出正確的句子：", words: ["家裏", "媽媽", "在", "下午", "看電話", "，", "。"], answer: "下午，媽媽在家裏看電話。" },
    { type: "reorder", question: "請排列出正確的句子：", words: ["吃聖誕大餐", "餐廳", "在", "聖誕節", "我和家人", "，", "。"], answer: "聖誕節，我和家人在餐廳吃聖誕大餐。" },
    { type: "reorder", question: "請排列出正確的句子：", words: ["圖書館", "我和同學", "小息", "在", "看書", "，", "。"], answer: "小息，我和同學在圖書館看書。" },
    { type: "reorder", question: "請排列出正確的句子：", words: ["晚上八時", "睡房", "弟弟", "做功課", "在", "，", "。"], answer: "晚上八時，弟弟在睡房做功課。" },
    { type: "reorder", question: "請排列出正確的句子：", words: ["抹窗戶", "在", "家裏", "星期六", "我和姐姐", "，", "。"], answer: "星期六，我和姐姐在家裏抹窗戶。" },
    { type: "reorder", question: "請排列出正確的句子：", words: ["早上七時", "在", "做早操", "叔叔", "公園", "，", "。"], answer: "早上七時，叔叔在公園做早操。" }
];
...
function startGame() {
    ...
    // ✅ 顯示控制區 (使用 setProperty 來確保覆蓋 CSS 的 !important)
...
for (let i = 0; i < 110; i++) {
    ...
    words.forEach((w, i) => {
        ...
        if (isGoal) {
            ...
            cell.innerHTML = `
                ${numberHtml}
                ${w !== "" ? `<div class="cell-text">${w}</div>` : ""}
                <span class="event-icon" style="${w !== "" ? 'position: absolute; bottom: 5px; right: 8px; font-size: 24px;' : 'font-size: 50px;'}">${iconText}</span>
            `;
        }
...
function showModal() {
   ...
   // 3. 根據題型生成 HTML 內容
    ...
    if (task.gif) { // 看圖配詞
        html += `<img src="images/${task.gif}" style="width:200px; border-radius:10px; margin-bottom:10px;">`;
    }
    if (task.sentence) { // 閱讀理解
        html += `<div style="background:#f0f7ff; padding:15px; border-radius:10px; margin-bottom:10px; text-align:left;">${task.sentence}</div>`;
    }
...
function checkUserAnswer(userSelect, correctAnswer) {
    ...
    const clickedBtn = currentEvent ? currentEvent.target : null;

    if (userSelect === correctAnswer) {
        // ✅ 答對了
        if (typeof playSound === "function") playSound('win');
        if (feedbackText) {
            feedbackText.innerHTML = "🌟 太棒了！你觀察得很仔細！";
            feedbackText.style.color = "#48bb78";
        }
        // 隱藏原本的選項，只顯示綠色的「完成」按鈕
        const actions = document.getElementById('modal-actions');
        if (actions) {
            actions.innerHTML = `<button onclick="closeModal()" class="finish-btn">完成挑戰 🏆</button>`;
        }
    } else {
        // ❌ 答錯了
        attempts--;
        ...
        if (attempts > 0) {
            if (feedbackText) {
                feedbackText.innerHTML = `😮 差一點點！還有 ${attempts} 次機會，加油！`;
                feedbackText.style.color = "#f56565";
            }
            // 答錯抖動特效 (需配合 CSS)
            if (clickedBtn && clickedBtn.classList.contains('opt-btn')) {
                clickedBtn.classList.add('wrong-answer');
                setTimeout(() => clickedBtn.classList.remove('wrong-answer'), 500);
            }
        } else {
            if (feedbackText) {
                feedbackText.innerHTML = "💔 沒關係，下次再挑戰！正確答案是「" + correctAnswer + "」";
                feedbackText.style.color = "#5d4037";
            }
            // 機會用完，等待 2 秒讓學生看清楚答案後自動關閉
...
function addToReorder(btn) {
    ...
    newBtn.className = "opt-btn"; 
    newBtn.innerText = btn.innerText;
    newBtn.style.color = "#5d4037"; // 確保字體是深色
    newBtn.style.margin = "4px";
...
function closeModal() {
    ...
    // 呼叫換人函數
    if (typeof finishTurn === "function") finishTurn();
}
    // 切換回合的函數 (獨立存放，不要包在別人裡面)
function finishTurn() {
    // 1. 切換玩家 (0 變 1, 1 變 0)
    turn = (turn === 0) ? 1 : 0;
    ...
    if (tn) {
        ...
        tn.style.color = (turn === 0) ? "var(--p1-color)" : "var(--p2-color)";
    }
...
function testModal(targetType) {
    ...
    if (filteredTasks.length > 0) {
        ...
        showModal();                            // 執行顯示
        window.allTasks = originalAllTasks;     // 恢復題庫
    } else {
